document.addEventListener("DOMContentLoaded", () => {
    class Tinipix {
        ...
        dragMove = (event) => {
            if (this.lastCursorX === undefined || this.lastCursorY === undefined) {
                this.lastCursorX = event.clientX;
                this.lastCursorY = event.clientY;
            }
        
            const dx = event.clientX - this.lastCursorX; // Change in X position
            const dy = event.clientY - this.lastCursorY; // Change in Y position
            ...
            if (this.isDragging) {
                this.position.x = event.clientX - this.element.offsetWidth / 2;
                this.position.y = event.clientY - this.element.offsetHeight / 1;
                this.updatePosition();
        
                // Flip only when there's a clear movement direction
                if (Math.abs(dx) > 1) { 
                    this.setScale(dx > 0 ? 1 : -1);
                }
            }
        
            // Store last cursor position for the next move event
            this.lastCursorX = event.clientX;
            this.lastCursorY = event.clientY;
        };

        stopDrag = () => {
            this.isDragging = false;  // Properly reset dragging
            document.removeEventListener("mousemove", this.dragMove);
            document.removeEventListener("mouseup", this.stopDrag);
            document.removeEventListener("mouseleave", this.stopDrag); // NEW: Ensure full cleanup
